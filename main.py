...
def main():
    ...
    db_create.add_argument('-e', '--engine_id', help='ID of the database engine', default=ENGINES_ID[0]["id"], required=False)
    db_create.add_argument('--volume_size', type=int, help='Size of the volume in GB', default=10, required=False)
    ...
    machine_list = machine_subparsers.add_parser('list', help='List all machine profiles')
    machine_create = machine_subparsers.add_parser('create', help='Create a new machine')

    # Adicionando argumentos para o comando machine create
    machine_create.add_argument('-n', '--name', help='Name of the virtual machine')
    machine_create.add_argument('-m', '--machine_name', help='Machine type name', default=MACHINES[0]["name"], required=False)
    machine_create.add_argument('-i', '--image_name', help='Image name', default=SYSTEMS[0]["name"], required=False)
    machine_create.add_argument('-k', '--ssh_key_name', help='SSH key name', required=True)
    ...
    if args.command == 'db':
        if args.subcommand == 'list':
            ...
            for profile in profiles:
                print(f"- {profile['label']} (ID: {profile['id']}, vCPU: {profile['vcpu']}, RAM: {profile['ram']}GB, Application: {profile['application']})")
    ...
    # Lógica para o comando machine
        ...
        if args.subcommand == 'list':
            ...
            for profile in profiles:
                print(f"- {profile['name']} (RAM: {profile['ram']}GB, vCPUs: {profile['vcpus']}, Disk: {profile['disk']}GB, GPU: {profile['gpu']})")
        elif args.subcommand == 'create':
